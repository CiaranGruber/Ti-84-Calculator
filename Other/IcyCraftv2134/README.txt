IcyCraft version 2.13.4 (TI-84+CE, TI-83pCE, TI-83pCE-EP)
...
Press [graph] to change the behavior pack
      This will prompt for an appvar name.
Press [mode] to clone the currently selected world.
      This will prompt for a 6 character world name to copy into.
...
--NEW WORLD--
When creating a new world, the game will now prompt for a seed. Input a string of text (max 16 chars) and press enter.
You may skip this process by pressing [clear], or inputting an empty string, to just have a random seed.
...
"looking at" the block you are currently pointing to. If you are not moving, this is the same as the block you are standing on.
"(<block>)" the block you are currently standing on.
The hearts are your health.
The screen will flash red if you take damage.
"x#####,y###,z#####" your current position in the world. X,Z are E/W, S/N. Y is up/down.
The box above the game view is your Exp points. You will recieve Exp from collecting ores.


--SURVIVAL MODE--
In this game mode, there is hunger; which saps the player's health.
The screen flashes red when you are taking damage.
You will eventually die and respawn if you run out of health. (hearts above the hotbar)
Respawn happens immediately, but you lose all of the items in your inventory. Permanently.
...
[enter]         Command window. Currently a dummy screen.
[zoom]          Copy the block you are facing and add it to your inventory.
...
v2.13.4:
+ Added Exp points!
+ Added in-game (inventory) instructions
* Tweaked the generator, because structures weren't generating as often as anticipated.
* Changed the seed hashing method.
v2.13.3:
+ Added seeds! Now the game can generate the same world over and over again! :D (provided an identical seed, of course)
* And of course tweaked the generator to allow seeds to work correctly.
v2.13.2:
* Fixed a game breaking bug where the wrong chests would open; from the player position rather than the chest's position.
+ Fixed the game reacting too fast to input.
v2.13.0:
+ Added garbage collect handler. Hopefully this triggers correctly.
+ Added Gravel hills biome. Spawns above Vertical level 64.
* Tweaked the world generator again.
* Default 'block broken' for upward break now uses hardcoded tileIDs. Above 64: nothing. At 64: grass. Below 64: Stone floor.
* Fixed structures spawning on incorrect vertical levels. (Hopefully)
v2.12.0:
